...
function MediaRow({
  ...
  onChange: (m: PortfolioMedia[]) => void;
}) {
  ...
  return (
    <div className="space-y-3">
      ...
      {media.map((m, i) => (
        <div
          key={m.id}
          className="flex gap-2 items-center bg-surface-container rounded-xl p-3"
        >
          <span
            className={`material-symbols-outlined flex-shrink-0 ${m.media_type === "youtube" ? "text-red-500" : "text-primary"}`}
            style={{ fontSize: 20 }}
          >
          ...
          {m.media_url &&
            m.media_type === "youtube" &&
            getYouTubeId(m.media_url) && (
              // eslint-disable-next-line @next/next/no-img-element
              <img
                src={`https://img.youtube.com/vi/${getYouTubeId(m.media_url)}/mqdefault.jpg`}
          ...
          <button
            onClick={() => remove(i)}
            className="flex-shrink-0 text-error/50 hover:text-error"
          >
            <span
      ...
      {media.length === 0 && (
        <p className="text-xs text-on-surface-variant opacity-50 text-center py-2">
          أضف صور أو فيديوهات للعمل
        </p>
      )}
    </div>
  );
...
function PortfolioModal({
  ...
  onSave: (d: Partial<Portfolio>) => void;
  onClose: () => void;
  lang: "ar" | "en";
}) {
  const isRTL = lang === "ar";
  ...
  const [f, setF] = useState<Partial<Portfolio>>({
    ...
    visit_link: "",
    order_index: 0,
    ...initial, // هنا الـ initial لو جواه media هيغطي على القيم اللي فوق، وده الصح
  });
        ...
        <div className="overflow-y-auto p-8 space-y-5 chat-scroll">
          ...
          <div>
            <label className="block text-xs font-label font-black uppercase tracking-widest text-on-surface-variant mb-3">
              {isRTL
                ? "الصور والفيديوهات (يمكن إضافة أكثر من واحد)"
                : "Media (you can add multiple)"}
            </label>
            <MediaRow
              media={f.media ?? []}
              onChange={(m) => setF((p) => ({ ...p, media: m }))}
            />
          </div>
...
export default function PortfolioAdminPage() {
  ...
  const getThumb = (item: Portfolio) => {
    ...
    if (first.media_type === "youtube") {
      const id = getYouTubeId(first.media_url);
      ...
      <div className="mb-10 flex items-end justify-between">
        <div>
          ...
          <p className="text-on-surface-variant">
            {isRTL
              ? "إدارة محفظة الأعمال مع صور وفيديوهات متعددة."
              : "Manage portfolio with multiple images & videos."}
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {items.map((item) => (
            <div
              key={item.id}
              className="bg-surface-container-lowest rounded-xl overflow-hidden border border-outline-variant/20 hover:shadow-lg transition-all"
            >
              <div
                className="relative bg-surface-dim"
